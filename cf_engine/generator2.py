...
(Python 3.11 이하 호환 버전)

[Python 3.11 이하 f-string 호환 규칙]
  규칙 1: f-string {} 안에서 딕셔너리 접근(t['key'])이나
          함수 호출(abs(t['delta']))을 직접 쓰지 않는다.
          → 반드시 먼저 변수로 분리한 뒤 f-string에 사용한다.

          # ❌ Python 3.11 에서 SyntaxError 위험
          f"{t['original']:.1f}{unit}"
          f"{abs(t['delta']):.1f} ({abs(t['pct_change']):.0f}%)"
          ...
          pct_abs   = abs(t['pct_change'])
          f"{delta_abs:.1f} ({pct_abs:.0f}%)"

  규칙 2: f-string {} 안에서 백슬래시(\) 절대 사용 금지.
          → 개행문자나 유니코드 이스케이프는 반드시 변수로 분리한다.

          # ❌ Python 3.11 이하 SyntaxError
          f"{chr(10).join(items)}"   # 우회 가능하지만 비권장
  ...
  Python 3.12부터는 위 제약이 모두 해제되어 generator.py 방식이 동작함.
"""
...
class TabDiffCFGenerator:
    ...
    def generate_for_batch(
        ...
        x_factual_repeated = np.repeat(x_factual_np, num_candidates, axis=0)
        ...
        for i in range(n):
            ...
            cf_list.append(self.preprocessor.inverse_transform(x_cf_np[start:end]))
        return cf_list

    def select_best_cf(self, cf_candidates: pd.DataFrame,
                        factual_row: pd.Series) -> pd.Series:
        """후보 중 최적 CF 선택: 활성(0) 예측 + 원본과 거리 최소"""
        x_factual_np = self.preprocessor.transform(
    ...
    def explain(self, customer_df: pd.DataFrame,
        ...
        for i in iterator:
            ...
            customer_id = row.get("customer_id", "고객_" + str(i + 1))  # 규칙1: + 연산으로 대체
            ...
            results.append({
                ...
                "factual_prob":   float(risk_probs[i]),
                "cf_prob":        cf_prob,
                "cf_valid":       cf_valid,
...
def _translate_cat(feat: str, val: str) -> str:
    trans = {
        "card_tier": {
            "The": "The Card", "Black": "Black", "Red": "Red", "Blue": "Blue",
        },
        "primary_spending_category": {
            "dining": "외식", "shopping": "쇼핑", "travel": "여행",
            "convenience": "편의점", "online": "온라인",
        },
        "payment_method": {
            "app": "앱결제", "online": "온라인결제",
...
def print_cf_report(result: Dict) -> None:
    ...
    risk_prob    = result["factual_prob"]
    cf_risk_prob = result["cf_prob"]
    ...
    # 규칙1: 포맷 값을 변수로 분리한 뒤 f-string 사용
    risk_pct     = risk_prob * 100
    cf_risk_pct  = cf_risk_prob * 100
    drop_pct     = (risk_prob - cf_risk_prob) * 100
    missed_int   = int(missed)
    days_app_int = int(days_app)

    print("\n" + "=" * 68)
    print(f"  고객 ID: {cid}  |  카드 등급: {tier}  |  거래 연수: {yrs:.1f}년")
    print("=" * 68)
    print(f"  현재 무실적 위험도: {risk_pct:.1f}%  "
          f"(최근 무실적 {missed_int}개월, 앱 미사용 {days_app_int}일)")
    print()
    ...
    if not treatments:
        ...
        print(f"  목표 달성 시 무실적 위험도: {cf_risk_pct:.1f}%  "
              f"(↓{drop_pct:.1f}%p 감소)")
        print()
        ...
        for t in treatments:
            ...
            unit      = t["unit"]
            # 규칙1: 딕셔너리 값을 변수로 먼저 꺼냄
            orig      = t["original"]
            cf        = t["counterfactual"]
            direction = t["direction"]
            pct       = t["pct_change"]
            delta_abs = abs(t["delta"]) if t["delta"] is not None else 0.0
            ...
            if t["type"] == "numerical":
                if unit in ("만원", "P"):
                    ...
                    cf_str     = f"{cf:,.0f}{unit}"
                    change_str = f"{direction}{delta_abs:,.0f} ({pct_abs:.0f}%)"
                elif unit in ("배",):
                    orig_str   = f"{orig:.2f}{unit}"
                    cf_str     = f"{cf:.2f}{unit}"
                    change_str = f"{direction}{delta_abs:.2f} ({pct_abs:.0f}%)"
                else:
                    orig_str   = f"{orig:.1f}{unit}"
                    cf_str     = f"{cf:.1f}{unit}"
                    change_str = f"{direction}{delta_abs:.1f} ({pct_abs:.0f}%)"
            else:
                feat_key = t["feature"]
                orig_str   = _translate_cat(feat_key, str(orig))
                cf_str     = _translate_cat(feat_key, str(cf))
...
def _print_wakeup_actions(treatments: List[Dict], factual: pd.Series) -> None:
    ...
    for t in treatments:
        ...
        if feat == "days_since_app_login" and direction == "↓":
            ...
            days_int = int(days)    # 규칙1: int() 호출을 변수로 분리
            actions.append(f"  → [앱 재참여] 푸시 알림 발송 / {days_int}일 미접속 특별 혜택 안내")
            ...
            pnt_int = int(pnt)      # 규칙1: int() 호출을 변수로 분리
            actions.append(f"  → [혜택 등록] 미가입 서비스 안내 / 포인트 {pnt_int:,}P 활용 혜택 추천")
            ...
            cf_val = t["counterfactual"]    # 규칙1: 딕셔너리 접근을 변수로 분리
            actions.append(f"  → [디지털 참여] 앱 전용 이벤트 / 간편결제 전환 유도 ({cf_val} 수준 목표)")
        elif feat == "payment_method":
            raw_val = t["counterfactual"]   # 규칙1
            cf_val  = _translate_cat("payment_method", str(raw_val))
            actions.append(f"  → [결제 수단] {cf_val} 전환 시 추가 적립 혜택 안내")
        elif feat == "primary_spending_category":
            raw_val = t["counterfactual"]   # 규칙1
            cf_val  = _translate_cat("primary_spending_category", str(raw_val))
...
def build_treatment_rows(result: dict) -> dict:
    ...
    factual = result["factual"]
    # 규칙1: 딕셔너리 접근은 변수로 분리
    card_tier    = factual.get("card_tier", "-")
    years_cust   = factual.get("years_as_customer", 0)
    engage_level = factual.get("engagement_level", "-")
    ...
    row = {
        ...
        "card_tier":         card_tier,
        "years_as_customer": years_cust,
        "engagement_level":  engage_level,
    }
    for feat in NUMERICAL_FEATURES:
        ...
        row[feat + "_orig"]  = orig
        row[feat + "_cf"]    = cf
        row[feat + "_delta"] = round(cf - orig, 2)
        row[feat + "_pct"]   = round((cf - orig) / orig * 100, 1) if orig != 0 else 0.0
    for feat in CATEGORICAL_FEATURES:
        orig_val = str(result["factual"].get(feat, ""))
        cf_val   = str(result["counterfactual"].get(feat, ""))
        row[feat + "_orig"]    = orig_val
        row[feat + "_cf"]      = cf_val
        row[feat + "_changed"] = int(orig_val != cf_val)
...
def build_action_rows(result: dict) -> list:
    ...
    for t in result["treatments"]:
        ...
        if t["type"] == "numerical":
            ...
            cf_str   = f"{cf_val:.1f}{unit}"
            change   = f"{direction}{delta_abs:.1f} ({pct_abs:.0f}%)"
        else:
            orig_raw = t["original"]
            cf_raw   = t["counterfactual"]
            orig_str = _translate_cat(feat, str(orig_raw))
            cf_str   = _translate_cat(feat, str(cf_raw))
            change   = "→ " + cf_str   # 규칙2: 백슬래시 없이 문자열 연결
